  ...
  var ImageDialog = function (summernote) {
    ...
    this.initialize = function () {
      ...
      var body = '<div class="form-group note-group-select-from-files">' +
                   ...
                   '<input class="note-image-input input" type="file" name="files" accept="image/*" multiple="multiple" />' +
                   imageLimitation +
                 '</div>' +
                 '<div class="form-group">' +
                   '<label>Image URL</label>' +
                   '<input class="note-image-url input" type="text" />' +
                 '</div>';
      var footer = '<button href="#" class="btn focus note-image-btn disabled" disabled>Insert Image</button>';
    ...
    this.showImageDialog = function () {
      return $.Deferred(function (deferred) {
        ...
        self.$dialog.jui('off');
        self.$dialog.jui('on', 'show', function () {
          // Cloning imageInput to clear element.
          $imageInput.replaceWith($imageInput.clone()
            .on('change', function () {
              deferred.resolve(this.files || this.value);
              self.$dialog.jui('hide');
            })
            .val('')
          );
        ...
        self.$dialog.jui('off', 'hide');
        self.$dialog.jui('on', 'hide', function () {
